...
<div id="comment-{{ $comment->getKey() }}" class="media">
    <img class="mr-2 mt-2 img-thumbnail" src="{{ asset('storage/avatar/' . $comment->commenter->avatar) }}" alt="">
    ...
    <div class="media-body">
        <h6 class="mt-2 mb-0">{{ $comment->commenter->firstname ?? $comment->guest_name }}</h6>
        ...
        <div>
            @can('reply-to-comment', $comment)
                <button data-toggle="modal" data-target="#reply-modal-{{ $comment->getKey() }}"
                    class="commentButton text-uppercase">@lang('comments::comments.reply')</button>
            @endcan
            @can('edit-comment', $comment)
                <button data-toggle="modal" data-target="#comment-modal-{{ $comment->getKey() }}"
                    class="commentButton text-uppercase">@lang('comments::comments.edit')</button>
            @endcan
            @can('delete-comment', $comment)
                <a href="{{ route('comments.destroy', $comment->getKey()) }}"
                    ...
                    class="commentButton text-danger text-uppercase">@lang('comments::comments.delete')</a>
